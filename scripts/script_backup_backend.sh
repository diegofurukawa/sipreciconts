...
BACKUP_DIR="$HOME/GitHub/sipreciconts/backup/backend/$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"

# Padrões de exclusão (diretórios e arquivos)
EXCLUDE_DIRS=( "backup/backend" "frontend" "backup" "migrations" "Feature_Requests" "Diversos" "diagrams" "Dev_Diary" ".pytest_cache" "__pycache__" "logs" "Logs" "old" "import" ".venv" "venv" "backup" "export" "documentation" "node_modules" )
